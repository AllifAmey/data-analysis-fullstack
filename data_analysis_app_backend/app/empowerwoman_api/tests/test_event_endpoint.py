...
class TestEventEndpoint(TestCase):
    """Test functionality of the event endpoint"""

    def test_event_endpoint(self):
        client = APIClient()
        user_details = {'username': 'test@example.com', 'password': '123'}
        user_admin_details = {
                'username': 'Rokhsareh@empoweredwomen-mcr.org',
                'password': '123'}
        ...
        admin_user = get_user_model().objects.get(
                username=user_admin_details['username'])
        ...
        test_event_1 = models.Event.objects.\
            create(
                    title="test",
                    description="d",
        ...
        """
        Test for PATCH request return model
        and the status code is correct with admin
        or specific email
        """
        res = client.patch(
                "/api/empowerwoman/event/1/",
